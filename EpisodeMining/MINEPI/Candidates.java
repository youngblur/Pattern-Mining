package MINEPI;

import java.util.ArrayList;
import java.util.List;

public class Candidates {
    // the list of the start position (index) of the block of having same n-1 prefix in the a level
    List<Integer> block_start;

    List<int[]> candidates;

    int candidateCount = 0;

    int K;


    Candidates(int K){
        this.candidates = new ArrayList<>();
        this.block_start = new ArrayList<>();
        this.K = K;
    }

    public void addCandidate(int[] candidate, int block_index){
        this.candidates.add(candidate);
        this.block_start.add(block_index);
        candidateCount++;
    }

    public boolean isEmpty(){
        return candidates.size()<=0;
    }

    public int getCandidateCount(){
        return this.candidateCount;
    }


    public void getFrequentKepisodes(List<Event> sequence,int minSupport,int maxWin, FreEpisodes freEpisodes){

        // the last candidate's block_start
        int last_block_start = -1;
        // the current frequentEpisode's block_start
        int current_block_start = -1;
        // We use k to record the number of frequent episode
        // In the paper, k = 0 , we  use k = -1.
        int k=-1;
        for(int i = 0; i<this.candidateCount; i++){

            List<FSA4MINEPI> FSAList = new ArrayList<>();
            FSAList.add(new FSA4MINEPI(this.candidates.get(i)));

            int support = 0;

            for(Event eventSet : sequence) {
                int currentFSAsize = FSAList.size();
                for(int j = currentFSAsize-1; j>=0; j--){
                    if(eventSet.contains(FSAList.get(j).waiting4Event())) {
                        FSAList.get(j).transit();
                        if (j == currentFSAsize - 1) {
                            // The j-th FSA is the first start
                            FSAList.get(j).addStartTime(eventSet.getTime());
                            // and we add a new FSA
                            FSAList.add(new FSA4MINEPI(this.candidates.get(i)));
                        }
                        if (FSAList.get(j).isEnd()) {
                            // if current FSA reach end state, and if it statify maxWin then support ++ ,finally remove it.
                            if (FSAList.get(j).getWinLength(eventSet.getTime()) <= maxWin) {
                                support++;
                            }
                            FSAList.remove(j);
                        }
                        if (j >= 1 && FSAList.get(j).isSame(FSAList.get(j - 1)) && !eventSet.contains(FSAList.get(j - 1).waiting4Event())) {
                            // if current FSA's wait equals to previous FSA's wait, then we delete previous FSA in the FSAList
                            FSAList.remove(j - 1);
                            j--;  // to index the current FSA in the FSAList
                        }
                    }
                }
            }
            if(support >= minSupport){
                k++;
                if(this.block_start.get(i) != last_block_start){
                    last_block_start = this.block_start.get(i);
                    current_block_start = k;
                }
                Episode episode = new Episode(candidates.get(i),support);
                freEpisodes.addFrequentFpisodeAndBlockStart(episode,this.K,current_block_start);
            }
        }
    }

}
